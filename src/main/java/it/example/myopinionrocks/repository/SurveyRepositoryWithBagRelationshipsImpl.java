...
public class SurveyRepositoryWithBagRelationshipsImpl implements SurveyRepositoryWithBagRelationships {
    ...
    public Optional<Survey> fetchBagRelationships(Optional<Survey> survey) {
        return survey.map(this::fetchSurveyQuestions);
    }
    ...
    public List<Survey> fetchBagRelationships(List<Survey> surveys) {
        return Optional.of(surveys).map(this::fetchSurveyQuestions).orElse(Collections.emptyList());
    }

    Survey fetchSurveyQuestions(Survey result) {
        return entityManager
            .createQuery("select survey from Survey survey left join fetch survey.surveyQuestions where survey.id = :id", Survey.class)
            .setParameter("id", result.getId())
            .getSingleResult();
    }

    List<Survey> fetchSurveyQuestions(List<Survey> surveys) {
        ...
        List<Survey> result = entityManager
            .createQuery("select survey from Survey survey left join fetch survey.surveyQuestions where survey in :surveys", Survey.class)
